...
test("Fabushi Chrome platform includes the product shell, browser bridge, and integrated userscript runtime", async () => {
  ...
  const userscriptRunner = await source("userscript-runner.js");
  assert.match(userscriptRunner, /CHATGPT_USERSCRIPT_UPDATE_URL/);
  assert.match(userscriptRunner, /loadRemoteRecord/);
  assert.match(userscriptRunner, /compareVersions\(remote\.version, current\.version/);
  await assert.rejects(source("userscript/chatgpt-auto-confirm.user.js"));
