...
Current browser release candidate: `0.6.19`. The bundled ChatGPT userscript is `2.9.45` and declares
...
## 0.6.19 loading recovery scheduler self-heal

The Chrome package bundles canonical userscript v2.9.45. Same-route loading recovery now performs a real reload instead of being cancelled as a no-op; cancelled navigation re-arms the scheduler; committed navigation has an 8-second no-unload watchdog; and recovery counters reset only after the loading signal really clears. Existing system keep-awake behavior is unchanged.
